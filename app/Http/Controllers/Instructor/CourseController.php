...
class CourseController extends Controller
{
    ...
    public function courseStudent($uuid)
    {
        $course = Course::where('user_id', auth()->id())->whereUuid($uuid)->firstOrFail();
        $data['title'] = __('Enroll Students');
        $data['course'] = $course;
        $data['enrollments'] = Enrollment::where('enrollments.course_id', $course->id)
        ...
        $data['navCourseActiveClass'] = 'active';
        return view('instructor.course.students', $data);
    }
