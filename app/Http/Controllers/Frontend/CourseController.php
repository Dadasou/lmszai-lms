...
class CourseController extends Controller
{
    ...
    public function filterCourseData($request)
    {
        if ($request->category_id) {
            $data['courses'] = Course::where('private_mode', '!=', 1)->whereCategoryId($request->category_id)->where(function ($q){
                $q->where('status', STATUS_APPROVED)
                    ->orWhere('status', STATUS_UPCOMING_APPROVED);
            });
        } elseif ($request->sub_category_id) {
            $data['courses'] = Course::where('private_mode', '!=', 1)->whereSubcategoryId($request->sub_category_id)->where(function ($q){
                $q->where('status', STATUS_APPROVED)
                    ->orWhere('status', STATUS_UPCOMING_APPROVED);
            });
        } else {
            $data['courses'] = Course::where('private_mode', '!=', 1)->where(function ($q){
                $q->where('status', STATUS_APPROVED)
                    ->orWhere('status', STATUS_UPCOMING_APPROVED);
            });
        }
