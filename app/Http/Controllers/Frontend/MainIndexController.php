...
class MainIndexController extends Controller
{
    ...
    public function contactUs()
    {
        ...
        $data['contactUsIssues'] = ContactUsIssue::where('status',STATUS_ACCEPTED)->get();
    ...
    public function filterQuery($request){
        ...
        if ($request->available_type) {
            $users->where(function ($q) use ($request) {
                if (in_array($request->available_type, [1, 2])) {
                    $q->where(function ($subQ) use ($request) {
                        $subQ->where('ins.available_type', $request->available_type)
                            ->orWhere('ins.available_type', 3)
                            ->orWhere('org.available_type', $request->available_type)
                            ->orWhere('org.available_type', 3);
                    });
                } else { // request is 3
                    $q->where(function ($subQ) {
                        $subQ->where('ins.available_type', 3)
                            ->orWhere('org.available_type', 3);
                    });
                }
            });
