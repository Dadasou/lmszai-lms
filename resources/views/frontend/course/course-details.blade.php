...
@push('script')
    ...
    <script>
        ...
        zai_player1.on('enterfullscreen', event => {
            $(".modal-dialog-centered").addClass('modal-fullscreen');
        });

        zai_player1.on('exitfullscreen', event => {
            $(".modal-dialog-centered").removeClass('modal-fullscreen');
        });
    </script>
    ...
    <script>
        ...
        const players = [...document.querySelectorAll('.js-player')].map(p => {
            const player = new Plyr(p);

            player.on('enterfullscreen', event => {
                $("#model-player").addClass('modal-fullscreen');
            });

            player.on('exitfullscreen', event => {
                $("#model-player").removeClass('modal-fullscreen');
            });
    ...
    <script>
        ...
        function enterFullScreenMode() {
            if (youtube_player.requestFullscreen) {
                youtube_player.requestFullscreen();
            } else if (youtube_player.mozRequestFullScreen) {
                youtube_player.mozRequestFullScreen();
            } else if (youtube_player.webkitRequestFullscreen) {
                youtube_player.webkitRequestFullscreen();
            } else if (youtube_player.msRequestFullscreen) {
                youtube_player.msRequestFullscreen();
            } else {
                // alert('in ios enter full screen');
                // iOS fallback
                youtube_player.classList.add('fullscreen-mode');
            }
            isInFullscreen = true;
            full_screen_button.innerHTML = '<i class="fa fa-compress"></i>';
        }

        function exitFullScreenMode() {
            if (document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement) {
                // If in native fullscreen mode
                ...
                // If in fake fullscreen (iOS fallback)
                // alert('in ios compress full screen');
                youtube_player.classList.remove('fullscreen-mode');
            }

            isInFullscreen = false;
            full_screen_button.innerHTML = '<i class="fa fa-expand"></i>';
        }
