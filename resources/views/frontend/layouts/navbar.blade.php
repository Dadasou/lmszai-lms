                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu {{$selectedLanguage->rtl == 1 ? 'dropdown-menu-end' : ''}}">
                                @foreach($categories as $category)
                                    <li>
                                        <a href="{{ route('category-courses', $category->slug) }}" class="dropdown-item @if(count($category->subcategories) > 0) dropdown-toggle @endif">{{ __($category->name) }}</a>
                                        @if(count($category->subcategories) > 0)
                                            <ul class="submenu dropdown-menu">
                                                @foreach($category->subcategories as $subcategory)
                                                    <li>
                                                        <a class="dropdown-item"href="{{ route('subcategory-courses', $subcategory->slug) }}">{{ __($subcategory->name) }}</a>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        @endif
